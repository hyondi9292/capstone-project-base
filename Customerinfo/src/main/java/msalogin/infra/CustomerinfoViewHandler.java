...
public class CustomerinfoViewHandler {
    ...
    public void whenCustomerRegistered_then_CREATE_1 (@Payload CustomerRegistered customerRegistered) {
        try {
            ...
            customerinfo.setCustomerStatus(customerRegistered.getStatus());
    ...
    //고객 해지
    @StreamListener(KafkaProcessor.INPUT)
    public void whenCustomerCancelled_then_UPDATE_1(@Payload CustomerCancelled customerCancelled) {
        try {
            ...
            if( customerinfoOptional.isPresent()) {
                ...
                customerinfo.setCustomerStatus(customerCancelled.getStatus()); 
                // view 레파지 토리에 save
                System.out.println("#########Customer Info Policy handler : Update(Customer Calcelled) Obj##########" + customerinfo);
                customerinfoRepository.save(customerinfo);
                }
